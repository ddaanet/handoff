#!/usr/bin/env bash
# PreToolUse hook for the Skill tool.
# When the activated skill is one of the two that author
# handoff-task.md — `handoff` or `precompact`, in bare or qualified
# form (the Skill tool accepts both as launches of the same skill) —
# wipe any prior handoff files so the skill runs against a clean slate.
# The skill itself then either writes a fresh handoff-task.md or leaves
# nothing (handoff's "nothing to hand off" case).
#
# Mechanical work — agent is not involved. Wipe+emit is shared with
# prompt-pre-hook.sh via _wipe-emit.sh; this script is just the
# Skill-tool filter on top.
set -euo pipefail

input="$(cat)"
tool_name="$(jq -r '.tool_name // ""' <<<"$input")"
[[ "$tool_name" == "Skill" ]] || exit 0

skill="$(jq -r '.tool_input.skill // ""' <<<"$input")"
# The Skill tool accepts both the bare and qualified name as launches
# of the same skill, so match both. Explicit allowlist (not a substring
# match) keeps unrelated skills no-ops.
case "$skill" in
    handoff | handoff:handoff | precompact | handoff:precompact) ;;
    *) exit 0 ;;
esac

exec bash "$(dirname "$0")/_wipe-emit.sh" "$(jq -r '.cwd // ""' <<<"$input")" "PreToolUse"
