...
write through, PostToolUse regenerated `handoff.md` from todo-list
junk, and the artifact was polluted.

The fix tightens both files to their owners:

- `handoff.md` is **fully hook-owned** (generated by the extract hook,
  read into context by the load hook). Agent Read / Write / Edit is
...
bypasses it. That gap is accepted — `handoff.md`'s content is injected
at SessionStart, so reads are pointless, and parsing arbitrary Bash
command strings to close the gap is fragile and out of scope.
