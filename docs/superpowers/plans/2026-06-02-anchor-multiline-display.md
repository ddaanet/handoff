...
**Goal:** Show up to 7 lines of assistant anchor text in full; collapse ≥8-line turns to first 3 + `[ N lines omitted ]` + last 3 (always hiding ≥2 lines); remove the stale 120-char line truncation from the text branch.
...
# 8-line: first 3 + [ N lines omitted ] + last 3, two middle lines absent.
...
# 8-line anchor: 3+[ N lines omitted ]+3, two middle lines absent.
...
# [ N lines omitted ] appears, and in correct order: L3 < [ N lines omitted ] < L6.
l3_line="$(grep -n 'ANCHOR8_L3' "$out" | head -1 | cut -d: -f1)"
ellipsis_line="$(grep -nF '[ N lines omitted ]' "$out" | head -1 | cut -d: -f1)"
...
[[ -n "$l3_line" && -n "$ellipsis_line" && -n "$l6_line" \
    ...
    || fail "anchor-multiline: expected L3 < [ N lines omitted ] < L6 order"
```
...
ANCHOR_HEAD_LINES = 3    # lines shown before [ N lines omitted ]
ANCHOR_TAIL_LINES = 3    # lines shown after [ N lines omitted ]
```
...
def clamp_anchor_lines(lines: list[str]) -> list[str]:
    if len(lines) <= ANCHOR_LINE_LIMIT:
        return lines
    return lines[:ANCHOR_HEAD_LINES] + ["[ N lines omitted ]"] + lines[-ANCHOR_TAIL_LINES:]
```
...
git commit -m "feat: show multi-line assistant anchor text (3+[ N lines omitted ]+3 at ≥8 lines)"
```
...
- ✅ ≥8-line text: first 3 + `[ N lines omitted ]` + last 3
